...
首先，我们明确这四个词描述的是两个不同维度的东西：

1. **同步 vs 异步**：关注的是 **消息通信机制** 或 **结果获取方式**。
2. **阻塞 vs 非阻塞**：关注的是 **程序在等待结果时的状态**。

可以把一次I/O操作（比如读文件、网络请求）分为两个阶段：

1. **等待数据就绪**：内核需要时间准备数据。
2. **数据拷贝**：将准备好的数据从内核空间拷贝到用户空间。

这四个概念主要描述的是在这两个阶段中，调用者和被调用者（通常是操作系统内核）是如何交互的。



| 组合方式                     | 等待数据阶段 (内核 -> 用户)  | 数据拷贝阶段 (内核 -> 用户) | 特点                               |
| :--------------------------- | :--------------------------- | :-------------------------- | :--------------------------------- |
| **同步阻塞**                 | **阻塞**                     | **阻塞**                    | 最常用，最简单，效率最低           |
| **同步非阻塞**               | **非阻塞** (需轮询)          | **阻塞**                    | 可做其他事，但轮询消耗CPU          |
| **异步阻塞 (I/O多路复用)**   | **阻塞**在 `select/epoll` 上 | **阻塞** (但很快)           | 高效处理多连接，复杂度高           |
| **异步非阻塞 (真正异步I/O)** | **非阻塞**                   | **非阻塞**                  | 理想模型，由内核完成所有工作后回调 |
...
- **同步/异步** 的核心是 **“结果如何被获取”**。
  - 同步：**主动等**，**等到底**。
  - 异步：**我先走**，**你叫我**。
- **阻塞/非阻塞** 的核心是 **“等待时线程在干嘛”**。
  - 阻塞：**线程睡觉**，不消耗CPU。
  - 非阻塞：**线程忙别的**（或忙轮询），消耗CPU。
