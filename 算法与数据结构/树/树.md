...
● 树 (Tree) - (最顶层的抽象概念)
  │
  ├─ ● 二叉树 (Binary Tree) - (一个“纲”：每个节点最多有两个子节点)
  │   │
  │   ├─ ● (按结构分类)
  │   │  ├─ ● 满二叉树 (Full Binary Tree): 每个节点要么是叶子，要么有两个子节点。
  │   │  └─ ● 完全二叉树 (Complete Binary Tree): 除了最后一层，所有层都是满的，且最后一层的节点都靠左排列。
  │   │
  │   └─ ● 二叉查找树 (Binary Search Tree, BST) - (一个“目”：在二叉树基础上增加了值的排序规则)
  │      │  (规则：左子节点 < 父节点 < 右子节点)
  │      │
  │      └─ ● 平衡二叉树 (Balanced Binary Tree) - (一个“科”：为了解决BST可能退化成链表的问题)
  │         │  (这是一个类别，而不是具体一种树)
  │         ├─ ● AVL树 (一种具体的平衡二叉树实现)
  │         └─ ● 红黑树 (另一种具体的平衡二叉树实现)
  │
  └─ ● B树 / B-树 (B-Tree) - (另一个“纲”：多路查找树，每个节点可以有多个子节点)
     │
     └─ ● B+树 (B+ Tree) - (B树的一个“变种”，为数据库和文件系统优化)
```
